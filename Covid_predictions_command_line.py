...
def plot_timeseries(case, country):
    ...
    plt.legend(labels)
    ax.set(title=f' Evolution of actual {case} in {country}', ylabel='Number of cases')
    ...
    f = px.bar(country1.sort_values(case,ascending = False), 
             x=country1.index, y=country1[case].values, color=f'{case}', labels=['Case'])
...
def Arima_fit(case, country, start_date= '2020-11-10', end_date='2020-11-17'):
    ...
    values_fit = np.log(country2[case].loc['2020-03-01':date_today].dropna().values)
    ...
    y_case = pd.DataFrame({f'{case}':country2[case].loc['2020-03-01':].dropna().values},index=country2[case]['2020-03-01':].index)
...
if __name__ == "__main__":
    ...
    case_input =input('choose between Confirmed, Recovered, Deaths, new_deaths, new_cases:')
    country_input = input('Give me a name of a Country, you would like to check Covid-19 analysis: ')
